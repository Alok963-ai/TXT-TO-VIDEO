...
my_name = "KAKA"
CHANNEL_ID = "-1002595680027"##change it with your channel 🆔 
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            try:  
                ...
                cc = f'**🎞️ VID_ID: {str(count).zfill(3)}.\n\nTitle: {name1} @bhairakaka_contact_bot {res}.mkv\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦📖🇮🇳📖✦━━━━━**'
                cc1 = f'**📁 PDF_ID: {str(count).zfill(3)}.\n\nTitle: {name1} @bhairakaka_contact_bot.pdf\n\n📚 Batch Name: {b_name}\n\n📥 Extracted By : {CR}\n\n**━━━━━✦📖🇮🇳📖✦━━━━━**'
        ...
        # Check if the response status is OK
            ...
            # Remove the PDF file after sending
                    ...
                    Show = f"📥 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝𝐢𝐧𝐠 »\n\n📝 Title:- `{name}\n\n**🔗 𝐓𝐨𝐭𝐚𝐥 𝐔𝐑𝐋 »** ✨{len(links)}✨\n\n⌨ 𝐐𝐮𝐥𝐢𝐭𝐲 » {raw_text2}`\n\n**🔗 𝐔𝐑𝐋 »** `{url}`\n\n**𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ @bhairakaka_contact_bot"
                    prog = await m.reply_text(Show)
                    res_file = await helper.download_video(url, cmd, name)
                    filename = res_file
                    await prog.delete(True)
...
async def txt_handler(bot: Client, m: Message):
    ...
    await input.delete(True)
    file_name, ext = os.path.splitext(os.path.basename(x))
    credit = f"@bhairakaka_contact_bot🇮🇳"
    try:    
        with open(x, "r") as f:
            content = f.read()
        content = content.split("\n")
